...
namespace PowerAim.Class.Native
{
    internal static class NativeAPIMethods
    {
        ...
        {
            if (PowerAim.Config.AppConfig.Current?.ToggleState?.HideUIFromCapture ?? true)
                HideForCapture(window.GetHandleSafe());
            return window;
        }
        ...
        {
            var exclude = PowerAim.Config.AppConfig.Current?.ToggleState?.HideUIFromCapture ?? true;
